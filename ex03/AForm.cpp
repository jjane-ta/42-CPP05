...
AForm::AForm(std::string name, int grade2sign, int grade2exec)
    : _name(name), _grade2sign(grade2sign), _grade2exec(grade2exec), _is_signed(false)
{
...
AForm::AForm(const AForm &aform)
    : _name(aform._name), _grade2sign(aform._grade2sign), _grade2exec(aform._grade2exec), _is_signed(false) {}
...
int AForm::getGrade2Exec(void) const { return (this->_grade2exec); }

bool AForm::isSigned(void) const { return (this->_is_signed); }
